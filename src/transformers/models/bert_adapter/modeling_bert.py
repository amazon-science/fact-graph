...
class T5GNNAdapt(nn.Module):
    ...
    def forward(self, hidden_states, graphs):
        ...
        y = F.elu(self.conv(input_gnn, graphs.edge_index, edge_type=graphs.y))
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        config_adapters.intermediate_size = 16
        print('size_adapt', config_adapters.intermediate_size)
        #self.adapter_graph = T5LayerAdapt(config_adapters)
        #self.adapter_graph = T5GNNAdapt(config_adapters)
        self.adapter = T5LayerAdapt(config_adapters)
        self.adapter2 = T5LayerAdapt(config_adapters)

        # self.adapter_graph = T5GNNAdapt(config_adapters)
        # self.adapter2_graph = T5GNNAdapt(config_adapters)
        self.adapter_graph = T5LayerAdapt(config_adapters)
        self.adapter2_graph = T5LayerAdapt(config_adapters)
    ...
    def forward(
        ...
        if graph:
            attention_output = self.adapter_graph(attention_output)
            #attention_output = self.adapter_graph(attention_output, graph)
        else:
        ...
        if graph:
            layer_output = self.adapter2_graph(layer_output)
            #layer_output = self.adapter2_graph(layer_output, graph)
        else:
...
class BertAdapterModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if context_emb is not None:
            embedding_output[:,0,:] = context_emb
